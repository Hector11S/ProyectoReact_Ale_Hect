...
namespace SIMEXPRO.API.Controllers.ControllersProduccion
{
    ...
    public class TallasController : ControllerBase
    {
        ...
        {
            var item = _mapper.Map<tbTallas>(TallasViewModel);
            var respuesta = _produccionServices.EliminarTalla(item);
            return Ok(respuesta);
        }

        [HttpGet("ListarByIdtalla")]
        public IActionResult ListByIdCategoria(int tall_Id)
        {
            var listado = _produccionServices.ListarSubCategoriasByIdtalla(tall_Id);
